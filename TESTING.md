...
- Shared mocks live in `src/test-preload.ts` (loaded via `bunfig.toml`): `obsidian`, a thin hand-rolled `parseYaml`, and the Obsidian UI primitives that import-time requires pretend to exist. `@octokit/rest` is mocked per-test by constructing fake `GitHubApiGateway` instances.
...
- **Real GitHub API.** No PAT in CI, no iOS CI at all, and the API would make the suite flaky and slow. The seam is narrow (`GitHubApiGateway`), the surface is six methods, and Octokit is a well-tested library. Mocking `GitHubApiGateway` and trusting Octokit is the right trade.
