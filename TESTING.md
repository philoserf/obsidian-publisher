...
- Octokit is mocked at three different levels, deliberately. `@octokit/rest` and `@octokit/request-error` are `mock.module`'d globally in the preload. `github-api-gateway.test.ts` then builds a **real** `GitHubApiGateway` and overwrites its private `octokit` field with a fake, injecting a recording `sleep` mock so both the attempt count and the number of backoffs between them are asserted without waiting (#312). Only `publisher.test.ts` mocks the gateway wholesale, and it does so through `Publisher`'s fifth constructor argument (typed `PublishGateway`) rather than by overwriting a private field, so the fake is type-checked against the four methods `Publisher` calls.
